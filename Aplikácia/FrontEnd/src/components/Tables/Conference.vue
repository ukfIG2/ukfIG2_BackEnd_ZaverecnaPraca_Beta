...
async submitEditForm() {
  ...
  try {
    await axios.put(`http://localhost/ukfIG2_ZaverecnaPraca_Beta/Aplikácia/BackEnd/public/api/conferences/${this.editingConference.id}`, this.editingConference);
...
async deleteConference(id: number) {
  try {
    await axios.delete(`http://localhost/ukfIG2_ZaverecnaPraca_Beta/Aplikácia/BackEnd/public/api/conferences/${id}`);
...
async changeState() {
  console.log('Initial state:', this.editingConference?.state); // Add this line
